...
@app.route("/buscarPlanDeViaje.html", methods=['GET', 'POST'])
def browser_cerca():
    """
    ...
    if request.method == 'GET':
        return render_template('buscarPlanDeViaje.html', plan=None)
    elif request.method == 'POST':
        # Petición de plan
        if request.form['submit'] == 'Genera':
            logger.info("Enviando peticion de plan")

            # Content of the message
            contentResult = ECSDI['Generar_planes' + str(get_count())]

            # Graph creation
            gr = Graph()
            gr.add((contentResult, RDF.type, ECSDI.Genera_plan))

            # Add restricción origen
